...
################################################
# Script by @ChampionCyrille - 22/06/2018
# For Alqo 3.5.0.1
################################################
...
read -e -p "User that will run Alqo core /!\ case sensitive /!\ : " whoami
if [[ "$whoami" == "" ]]; then
...
#Create alqo.conf
decho "Setting up alqo Core" 
#Generating Random Passwords
user=`pwgen -s 16 1`
password=`pwgen -s 64 1`

echo 'Creating alqo.conf...'
mkdir -p /home/$whoami/.alqo/
cat << EOF > /home/$whoami/.alqo/alqo.conf
rpcuser=$user
...
echo 'alqo Core prepared and lunched'
...
echo "3- Once completed, please return to VPS and wait for the wallet to be synced.( alqo-cli mnsync status ) "
echo "4- Then you can try the command 'monoeci-cli masternode status' to get the masternode status."
echo "5- if the service rendered by this script does not hesitate to make a small donation ;)"
echo "6- Alqo adress : AP6iivqK5n6dDxm5ZBN8Qs7CQw1ov2n1a7"
su $whoami
